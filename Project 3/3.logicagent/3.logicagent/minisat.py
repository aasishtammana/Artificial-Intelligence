...
class AIMA_to_Dimacs_Translator(object):
    ...
    def to_dimacs_string(self, clauses):
        ...
        for clause in clauses:
            ...
            dimacs_vlist = []
            if clause.op == '|':
                for var in clause.argslist:
                    dimacs_vlist.append(('-' if var.op == '~' else '') \
                                        + self.varname_dict[var.argslist[0]
                                                            if var.op == '~' else var])

                ret += ' '.join(dimacs_vlist)
            elif clause.op == '~':
                ret += '-' + self.varname_dict[clause.argslist[0]]
            else:
        ...
        for clause in clauses:
            ...
            if clause.op == '|':
                ...
                if literal_name(clause) == literal_name(variable):
                    ...
                    ret_clause += '-' + self.varname_dict[clause.argslist[0]]
...
class Minisat(object):
    COMMAND = 'minisat %s %s > ' + os.devnull#'minisat %s %s > /dev/null'
